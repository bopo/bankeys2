...
function ForSession(message,statue) {
    $("#Initiative_Call_Div")/*.modal('hide');*/		// 隐藏正在呼叫层
	console.log("statue-------->"+statue);
	console.log("message-------->"+message);
	if(statue &&message == "客户端用户拒绝会话"){
		currentScope.sessionMessages = message;
		currentScope.refreshSessionMessages();
	}
	if(statue &&message == "会话结束..."){
		currentScope.sessionMessages = "会话结束...,相关信息已保存！";
		currentScope.refreshSessionMessages();
	}
...
function RejectRequestBtnClick() {
	...
	// $("#Initiative_Call_Div").modal().hide();//隐藏
	$("#enterRoom").show();//显示队列列表
...
function onVideoCallControlStart(dwUserId, dwErrorCode, dwFlags, dwParam, szUserStr)
{
    if (userType == USER_TYPE_AGNET) {//客服
		$("#Initiative_Call_Div").hide();//隐藏主动呼叫对话框   $("#Initiative_Call_Div").modal().hide();
		$('#LOADING_GREY_DIV').hide();
	}
    ...
    mRoomId = dwParam;
	if(errorcode==0){
		AUDIT_SWITCH1 = dwErrorCode;
		AUDIT_SWITCH2 = dwFlags;
		AUDIT_SWITCH = dwUserId;
	}
...
{	
	...
	if (userType == USER_TYPE_AGNET) {
		...
		currentScope.anyChatUserName = UserName;
	    ...
	    $("#Initiative_Call_Div").modal({backdrop: 'static', keyboard: false});
	    // $('#LOADING_GREY_DIV').show();
	    startServiceTag=false;
	}
}
