...
function OnAnyChatRecordSnapShotEx2(dwUserId, dwErrorCode, lpFileName, dwElapse, dwFlags, dwParam, lpUserStr) {
	...
	AddLog("录制完成回调函数执行"+dwFlags);
	AddLog(lpFileName+"录制或则拍照完成,时长为"+dwElapse);
	if(dwFlags=='23607'){
		//图片
		// $("#myimg").prop("src",lpFileName);
		// BRAC_TransFile(-1, dwElapse, 0, "0", dwFlags)
		AddLog("上传完成");
		console.log("tupian"+lpFileName)
		currentScope.saveToggleSnapshot(lpFileName);

		//判断浏览器是否支持FileReader接口
		// if(typeof FileReader == 'undefined'){
		// 	AddLog("浏览器不支持FileReader");
		// } else{
		// 	AddLog("浏览器NB");
		// }
	}else{
		//视频
		console.log("shipin"+lpFileName)
		currentScope.saveToBlob(lpFileName);
		// $("#mymv").prop("src",lpFileName);
	}
}
...
function OnAnyChatLoginSystem(dwUserId, errorcode) {
    ...
    if (errorcode == 0) {
        ...
        AddLog("OnAnyChatLoginSystem 正在加载营业厅，请稍候.....", LOG_TYPE_EVENT);
        switch (userType) {
            ...
            case USER_TYPE_AGNET:
                ...
                servantName = currentScope.offerUser.username; //客服登入账户名
			...
			//	var videoHtml = '  <div  id="remoteVideoDiv"><div style="background:transparent url(\'images/loading/07.png\') 50% 50% no-repeat ;object-fit: fill;padding-right: 0px;padding-left: 0px;height: 300px" class="col-md-12 borderDashed vodBox" id="remoteVideoPos"></div> <div style="width:568px;height:5px;" id="remoteAudioVolume"></div> </div> <div  > <div id="localVideoDiv" style="width:240px;padding-top:10px;"> <div  style="background:transparent url(\'images/loading/08.png\') 50% 50% no-repeat ;object-fit: fill;padding-right: 0px;padding-left: 0px;" class="col-md-4 borderDashed vodBoxT" id="localVideoPos"></div><div style="width:240px;height:5px;" id="localAudioVolume"></div></div></div>';
               // $("#videoCallContent").html(videoHtml); //填充视频会话层
				...
				//function stopService(){
					 if ($("#remoteVideoPos").html() != "") {
						 ...
						 $("#Initiative_Call_Div").modal().hide(); //隐藏主动呼叫层
						 isShowReturnBtn(true);
						 startServiceTag = false;
						 // }
					 }else{
						 // alert("请建立视频链接");
						 $('#comfirmMessages').text("请建立视频链接!").show();
                ...
                $('#startService').off().on('click', function () {
					if(queuesUserCounts == 0 ||queuesUserCounts == null){
						alert("没人")
					}else {
						document.getElementById("review_content").style.display = 'none';
						document.getElementById("review_contents").style.display = 'block';
						if (!startServiceTag) {
      ...
      AddLog("OnAnyChatLoginSystem 正在加载营业厅，请稍候.....111", LOG_TYPE_EVENT);
...
// 	alert("aaaa")
// 	new OnAnyChatLoginSystem().stopService();
//
// }

//按钮状态
var callButton = document.querySelector('button#startService');
var hangupButton = document.querySelector('button#hungUp');
var snapshotButton = document.querySelector('button#snapshot');
var screenshotButton = document.querySelector('button#screenshot');
	...
	if (errorcode == 0 && AUDIT_SWITCH!=1) {
	    if (userType == USER_TYPE_AGNET) {
			...
			var myName = BRAC_GetUserInfo(mSelfUserId,USERINFO_NAME);
	      	AddLog("OnAnyChatLoginSystem 客服ID->", mSelfUserId);
			AddLog("OnAnyChatEnterRoom 客户ID->", mTargetUserId);
			// currentScope = mTargetUserId;

			console.log("mTargetUserId----------------->"+mTargetUserId);
			console.log("USERINFO_NAME----------------->"+USERINFO_NAME);
			console.log("mSelfUserId----------------->"+mSelfUserId);
			...
			//
			var Terminal = BRAC_ObjectGetStringValue(ANYCHAT_OBJECT_TYPE_CLIENTUSER, mSelfUserId, ANYCHAT_OBJECT_INFO_STRINGTAG);

			currentScope.CustomerInfo(agentUserName,Terminal);
			
			AddLog("OnAnyChatLoginSystem 客服姓名->", agentUserName);
			AddLog("OnAnyChatEnterRoom 客户姓名->", myUserName);
			AddLog("OnAnyChatTerminal 终端信息->", Terminal);
			AUDIT_SWITCH1=mTargetUserId;
			AUDIT_SWITCH2=mSelfUserId;
			console.log("AUDIT_SWITCH1----------------->"+AUDIT_SWITCH1);
			console.log("AUDIT_SWITCH2----------------->"+AUDIT_SWITCH2);
			...
			/**个人姓名*/
			var myName = BRAC_GetUserInfo(mSelfUserId,USERINFO_NAME);
			AddLog("OnAnyChatLoginSystem 客服姓名", name1);
			AddLog("OnAnyChatEnterRoom 客服姓名", myName);
		...
		$("#Initiative_Call_Div").modal('hide');

		if(AUDIT_SWITCH!=1){
			if (callButton.textContent === '摘机') {
				callButton.textContent = '正在服务...'
				$(callButton).addClass("btnLef");
				callButton.disabled = true;

				// hangupButton.disabled = false;
				// $(hangupButton).removeClass("btnLef");

				// if (mTargetUserId == null) {
				// 	snapshotButton.disabled = false;
				// 	$(snapshotButton).removeClass("btnLef");
                //
					// screenshotButton.disabled = false;
					// $(screenshotButton).removeClass("btnLef");
				// }
				currentScope.toggleChangeState(2);
			}
				snapshotButton.disabled = false;
				$(snapshotButton).removeClass("btnLef");
			// 打开本地视频
			...
			/*视频开始录制*/
			//录制在本地 （默认保存路径C:\Users\sfb\AppData\Local\VirtualStore\Program Files (x86)\BaiRuiTech\AnyChatWeb\temp\record\2016-12-05）
			//var dwFlags = ANYCHAT_RECORD_FLAGS_VIDEO + ANYCHAT_RECORD_FLAGS_AUDIO + ANYCHAT_RECORD_FLAGS_MIXAUDIO +  ANYCHAT_RECORD_FLAGS_ABREAST;

			//在客户端合成后录制到服务器(var BRAC_RECORD_FLAGS_LOCALCB   = 0x00000800;  )
			//var dwFlags =BRAC_RECORD_FLAGS_LOCALCB + ANYCHAT_RECORD_FLAGS_VIDEO + ANYCHAT_RECORD_FLAGS_AUDIO + ANYCHAT_RECORD_FLAGS_MIXVIDEO + ANYCHAT_RECORD_FLAGS_MIXAUDIO + ANYCHAT_RECORD_FLAGS_SERVER + ANYCHAT_RECORD_FLAGS_STREAM;

			//在服务器合成录制到服务器
			var dwFlags = BRAC_RECORD_FLAGS_LOCALCB + ANYCHAT_RECORD_FLAGS_VIDEO + ANYCHAT_RECORD_FLAGS_AUDIO + BRAC_RECORD_FLAGS_STREAM + ANYCHAT_RECORD_FLAGS_ABREAST + ANYCHAT_RECORD_FLAGS_MIXAUDIO + ANYCHAT_RECORD_FLAGS_SERVER;
			AddLog("视频录制开始1");
...
function OnAnyChatRoomOnlineUser(dwUserCount, dwRoomId) {
	...
	if(AUDIT_SWITCH==1){
		console.log("mSelfUserId----------->"+AUDIT_SWITCH2+"--------------------->"+AUDIT_SWITCH1)
		// //请求本地视频
		// startVideo(AUDIT_SWITCH2, GetID("localVideoPos"), "ANYCHAT_VIDEO_LOCAL",1);
		//请求对方视频
		startVideo(AUDIT_SWITCH1, GetID("remoteVideoPos"), "ANYCHAT_VIDEO_REMOTE",1);
	}else {
		//请求对方视频
...
function OnAnyChatLinkClose(reason, errorcode) {
	...
	//$("#Session_Messages").modal();
	$("#sessionMessages").text("与服务器的网络连接异常中断！请重新加载页面！").show();
...
function jie(){
	if($("#remoteVideoPos").html()!=""){
		AddLog("开始抓拍"+mSelfUserId);
		//mSelfUserId     抓拍自己摄像头画面
		//mTargetUserId   抓拍对方摄像头画面

		//上传服务器 截屏
		var dwFlags = BRAC_RECORD_FLAGS_LOCALCB + ANYCHAT_RECORD_FLAGS_VIDEO + ANYCHAT_RECORD_FLAGS_AUDIO + BRAC_RECORD_FLAGS_STREAM + ANYCHAT_RECORD_FLAGS_MIXVIDEO + ANYCHAT_RECORD_FLAGS_MIXAUDIO + ANYCHAT_RECORD_FLAGS_SERVER;

		BRAC_SnapShot(mTargetUserId,dwFlags, 0);
	}
	// C:\Users\sfb\AppData\Local\VirtualStore\Program Files (x86)\BaiRuiTech\AnyChatWeb\temp\snapshot\2016-12-05     抓拍图片默认路径
}
...
function OnAnyChatVideoCallEvent(dwEventType, dwUserId, dwErrorCode, dwFlags,dwParam, szUserStr) {
	switch(dwEventType)
	{
		...
		case BRAC_VIDEOCALL_EVENT_FINISH:
			...
			callButton.textContent = '摘机'
			callButton.disabled = false;
			$(callButton).removeClass("btnLef");

			// hangupButton.disabled = false;
			// $(hangupButton).addClass("btnLef");

			// if(mTargetUserId==null){
				snapshotButton.disabled = true;
				$(snapshotButton).addClass("btnLef");

				// screenshotButton.disabled = true;
				// $(screenshotButton).addClass("btnLef");

				// currentScope.currentCustomer = null;
			// }
			currentScope.deleteAudit();//释放audit
			
			currentScope.toggleChangeState(0);

			currentScope.canncelChecked();


			/*视频停止录制*/
			//dwFlags = ANYCHAT_RECORD_FLAGS_VIDEO + ANYCHAT_RECORD_FLAGS_AUDIO + ANYCHAT_RECORD_FLAGS_SERVER + ANYCHAT_RECORD_FLAGS_STREAM + ANYCHAT_RECORD_FLAGS_MIXVIDEO + ANYCHAT_RECORD_FLAGS_MIXAUDIO;
			//dwFlags = ANYCHAT_RECORD_FLAGS_VIDEO + ANYCHAT_RECORD_FLAGS_AUDIO + ANYCHAT_RECORD_FLAGS_SERVER + ANYCHAT_RECORD_FLAGS_STREAM + ANYCHAT_RECORD_FLAGS_MIXVIDEO + ANYCHAT_RECORD_FLAGS_MIXAUDIO;
...
function OnAnyChatObjectSyncDataFinish(dwObjectType, dwObjectId) {
    ...
    if (dwObjectType == ANYCHAT_OBJECT_TYPE_AREA) {
        ...
        //自动进入营业厅
        hallbuinessNum = parseInt(areaIdArray[0]); //营业厅号为id
        /**进入营业厅*/
...
function OnAnyChatAgentWaitingUser(){
	...
	if($("#remoteVideoPos").html() == ""){
		// console.log("当前队列中------------->已没有客户")
		// alert("当前队列中已没有客户！");
		$('#comfirmMessages').text("当前队列中已没有客户！").show();
		$('#confirmModals').modal();
		// $('#LOADING_GREY_DIV').hide();
		startServiceTag=false;
		// $("#LOADING_GREY_DIV span").show();
