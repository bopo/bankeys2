...
def pip():
    ...
    local('pip install rongcloud.zip')
...
@task
def test(task=''):
    local('''DJANGO_SETTINGS_MODULE=config.settings.tests python manage.py test %s  --traceback -v2''' % task)
...
def pack():
    ...
    local('tar zcfv ../%s.tgz '
          ...
          '--exclude=.template '
          '--exclude=db.sqlite3 '
          '--exclude=Vagrantfile .' % datetime.date.today())
